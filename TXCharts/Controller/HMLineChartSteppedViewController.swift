//
//  HMLineChartSteppedViewController.swift
//  HEMS
//
...
class HMLineChartSteppedViewController: UIViewController, HMChartViewDelegate {

    private var stepppedChartView: HMLineChartView!
...
//        chartConfig.isNeedFillColor = true
    ...
    func bubbleViewContent(view: HMLineChartView, selectedIndex: Int) {
        if let xlist = chartData.xlist, let ylist = chartData.ylist {
            ...
            stepppedChartView.bubbleContent = NSAttributedString(string: content)
        }
    }
    ...
    private func setupUI() {
        ...
        // 创建自定义视图
        stepppedChartView = HMLineChartView()
        stepppedChartView.delegate = self
        stepppedChartView.setChartConfig(bgLineConfig, chartConfig: steppedChartConfig)
        
        // 添加自定义视图到当前视图控制器的视图中
        self.view.addSubview(stepppedChartView)
